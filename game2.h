...
void enemy_shot(vector<bullet_one> &bullets,unsigned long long int time,mychar &mc,WINDOW* gameplay);
void enemy_spawnshot(vector<bullet_one> &bullets,vector<enem> &enemys,unsigned long long int time,WINDOW* gameplay);
void enemy_shot(vector<bullet_two> &bullets,unsigned long long int time, mychar &mc,WINDOW* gameplay);
void enemy_spawnshot(vector<bullet_two> &bullets,vector<enem> &enemys,unsigned long long int time,WINDOW* gameplay);
...
void enemy_shot(vector<bullet_one> &bullets,unsigned long long int time, mychar &mc,WINDOW* gameplay) {
	for (int i = 0; i < bullets.size(); i++) {
		if (bullets[i].time+bullets[i].jeda<=time) {
			...
			if (hitCheck(bullets[i].x,bullets[i].y,mc,gameplay)) return;
			//hitCheck(i.x,i.y,mc,gameplay);
...
void enemy_spawnshot(vector<bullet_one> &bullets,vector<enem> &enemys,unsigned long long int time,WINDOW* gameplay) {
	for (enem &i: enemys) {
		if (i.time <= time) {
			...
			if (rand()%100>=80) {
				switch (rand()%4) {                                                          
					case 0: bullets.push_back({rand()%(lebar-2),0,ACS_DARROW,ACS_VLINE,i,20,time}); break;
					case 1: bullets.push_back({lebar-1,rand()%(tinggi-2),ACS_LARROW,ACS_HLINE,i,5,time}); break;
					case 2: bullets.push_back({rand()%(lebar-2),tinggi-1,ACS_UARROW,ACS_VLINE,i,5,time}); break; 
					case 3: bullets.push_back({0,rand()%(tinggi-2),ACS_RARROW,ACS_HLINE,i,20,time}); break;
				}
		...
		if (i.time+i.delay <= time) {
			...
			if (rand()%100>=0) {
				int x = 1+rand()%(lebar-2),
					y = 1+rand()%(tinggi-2);
				bullets.push_back({x,y,ACS_BULLET,ACS_DIAMOND,i,100,time});
